...
## 3. 什么是进程,什么是线程,他们之间是什么关系?  

1. 进程:
    - 执行程序时的一个实例  
   - 在每个进程都有独立的内存地址空间  
   - 系统进行资源分配何调度的基本单位  
   - 进程里的堆,是一个过程中最大的一块内存,被进程中的所有线程共享的,  
   进程创建时分配,主要存放new创建的对象实例  
    - 进程里的方法去,是用来存放进程中的代码片段的,是线程共享的  
...
2. 线程  
    ...
    - CPU调度何分配的基本单位  
    - 线程本身是不会独立存在  
    - 当线程CPU时间片用完后,会让CPU等下次轮到自己的时候再执行  
    - 系统不会为线程分配内存,线程组之间只能共享所属进程的资源
    - 线程只拥有再运行中必不可少的资源(如程序计数器,栈)
    - 线程里的程序计数器是为了记录该线程再让出CPU执行权的时候的执行地址,
      待再次分配到时间片的时候就可以自己私有的计数器指定地址继续执行
    - 每个线程都有自己的栈资源,用于存储该线程的局部变量何调用栈帧,其他线程无权访问  
    

3. 关系  

- 一个程序至少一个进程,一个进程至少一个西安测绘给你,进程中多个线程是共享进程资源的  
- java中当我们启动main函数时候就启动了一个JVM进程,而main函数所在的线程就是这个进程中的
一个线程,也叫主线程
  
- 一个进程中有多个线程,多个线程共享进程的堆何方法区资源,但是每个线程
有自己的程序计数器,栈区域
