...
对称加密又叫做私钥加密,即信息的放松方和接收方使用同一各密钥去加密和解析数据; 对称加密的特点是算法公开,加密和解密速度快,适合对大数据量进行加密
...
因为http的内容是明文传输的,明文数据会经过中间代理服务器,路由器,wifi热点,同i性能服务运营商等多个物理节点,如果信息在传输过程中被接池,传输的内容就会完全暴露了;  
劫持者还可以篡改传输的信息且不被双发察觉, 这就是"中间人攻击",所以我们才需要对信息进行加密;  
