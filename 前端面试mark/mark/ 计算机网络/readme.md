...
## 11 如何理解UDP和TCP? 区别? 应用场景?

### 11.1 UDP

UDP(User Datagram Protocol) 用户数据报协议,是一种简单的**卖你想数据报的通信协议**,即对应用层交来下的报文,不合并,不拆分

只是在其上面加上首部后就交给下面的网络层

也就是说无论应用较给`UDP`多长的报文,它统统发送,一次发送一个报文

而对接收方,接到后直接去除首部,交给上面的应用层完成任务;

`UDP`报头包括四个节点,每个字段占用2个字节(即16个二进制位),标题短开销小
...
- 并且它是将应用程序发来的数据在收到的那一刻,立刻按照原样发送到网络上的一种机制,即使出现网络拥塞的情况,UDP也无法机型流量控制灯避免网络拥塞行为
...
可以想像称流水形式的,发送方,TCP会将数据放入**蓄水池**(缓存区),等到可以发送的时候就发送,不能发送就等着,
TCP会根据当前网络的拥塞状态来确定每个报文段的大小  
`TCP`报文首部有20个字节,额外开销大
![img_22.png](img_22.png)
特点如下

### 12. 为什么TCp连接需要三次握手四次回收

**为什么是三次握手**  

**为了防止已失效的连接请求报文段突然又送到了服务器,因而产生错误;**

假设两次握手时,客户端发出的第一个请求连接报文再某一网络节点长事件滞留;
以导致延误到连接释放后才到达服务器;服务器收到失效的连接请求报文端后,
认为时客户端又发出一次新的连接请求,于是向客户端发送确认报文端,同意建立连接,此时在假定两次握手的前提下,连接接力陈公公,这样会导致服务器资源的浪费

**为什么时四次挥手**  

TCP协议是全双工通信,这意味着客户端和服务器端都可以向彼此发送数据,所以关闭连接时双方都需要确认的共同行为;  

假设是三次挥手是,首先释放课和护短到服务器方向的连接,此时TCP连接处于半关闭状态,这是客户不饿能向服务器发送数据,
而服务器还是可以向客户发送数据,如果此时客户收到了服务器的确认报文段后,就立即发送一个确认报文段,  

这回导致服务器向客户端还在发送数据式连接就关闭,这样也会导致客户端没有完整接受到服务器所发的报文段  
...
**什么是UDP**  
UDP是OSI参考模型中一种无连接的传输层协议,提供面向事物的简单不可靠信息传送服务

**区别:**

TCP是面向连接的传输控制协议,而UDP提供了无连接的数据保服务//类似于电话与短信  
TCP面向连接,提供可靠的数据服务  
TCP首部开销20字节,UDP首部开销8字节  
TCP逻辑通信信道是全双工的可靠信道,UDP则是不可信道  
UDP没有拥塞机制,因此网络出现拥塞不会使圆主机的发送效率降低,(有利于实时会议视频等)  
TCP连接之间能是点到点,UDP支持一对一,多对一,多对多的交互通信  
...
3. 负载均衡  
    rpc自带负载均衡  
   http需要自己去搞,比如nginx等等
   
4. 传输效率  
    可以自定义tcp协议,报文相对比较小  
   http有很多无用的东西,(包含了很多头部信息,keepalivetime,reffer)
   
5. 通知   
    rpc 自动通知
   http 事先通知,自行修改nginx配置或者其他负载均衡的配置
   
3. http1.0, 1.1 , 2.0 有什么区别  
    1.0 短链接
   1.1 长连接
   2.0 长连接+io多路复用
...
性能上有很大的问题,那就是没每发起一个请求,都要新建一次TCP连接(三次握手),
而且是串行请求,做了无谓的TCP连接建立和断开,增加了通信开销
...
- 优点:
- 简单: http基本的报文格式就是`header + body`,头部信息也是`key-value`简单文本的形式,易于理解,降低了学习和使用门槛
- 灵活易于扩展
    - http协议李德各类请求方法,UPI/UPL,状态码,头字段等每个组成要求都没有被固定四,都允许开发人员自定义和扩展
    - http 由于是工作在应用层(OSI第七层),则它下层可以随意变化
    
- 应用广泛和跨平台
    - http的应用范围和非常的广泛,从台式机的浏览器到手机上的各种APP,从看新闻,刷贴吧到购物,理财,http的应用遍地开花,同时天然具有跨平台的优越性
- 缺点
    - 无状态的双刃剑
        - 无状态的好处
            - 因为服务器不会去记忆HTTP的状态,所以不需要额外的资源来记录状态信息,这能减轻服务器的负担,
                能够把更多的CPU和内存用来对外提供服务
        - 无状态的坏处
            - 既然服务器没有记忆能力,它在完成有关联性的操作时会非常麻烦
                -   例如登录->添加购物车->阶段->支付, 这系列的操作都要知道用户的身份才行,但服务器不知道这些请求时有关联的,每次都要问一遍身份信息
        -  对于无状态的问题,解决的方案有很多种,其中比较简单的方式用cookie技术
        - cookie通过在请求和响应保卫呢种写入cookie信息来控制客户端的状态
    - 明文传输的双刃剑
      - 明文的好处:明文意味着在传输过程种的信息,方便阅读,装包可以直接可见,为调试工作带了极大的便利性
      - 明文的坏处:http的所有信息都暴露,相当于信息罗本,在传输的漫长过程中,信息的内容都毫无隐私可言,很容易就能被窃取
      - 安全问题: 
        - 通信使用明文(不加密),内容可能会被窃听
        - 不验证通信方的身份,因此可能会遭遇伪装
...
- http1.1性能优化
    - 使用长连接的方式改善了http1.0短连接造成的性能开销
         - 为了解决http1.0短连接开销答大题,http1.1提出了长连接的通信方式,也叫持久连接,这种方式的好处在于减少了TCP
            连接的重复建立和断开所造成的额外开销,减轻了服务器端的负载
         - 持久连接的特点: 只要任意端没有明确提出断开连接,则保持TCP连接状态
         - 如果某个HTTP长连接超过一定时间没有任何数据交互,服务器就会主动断开这个连接
    - 管道网络传输
         - 可以在统一个TCP连接里面,客户端可以发起多个请求,只要第一个请求发出去了,不必等其回来,就可以发第二个请求出去,可以减少整体的响应时间
        - 但是服务器必须按照接收请求的顺序发送对这些管道化请求的响应
        - 如果服务端在处理A请求的耗时比较长,那么后续的请求处理都会被阻塞住,这称为[队头阻塞],所以HTTP1.1管道解决了请求的队头阻塞,但是没有解决响应的队头阻塞
        - 实际上Http1.1管道化技术不是默认开启的,而且浏览器基本没有支持,**有这个功能,但是没有被用**
    - 响应队头阻塞:[请求-应答]的模式加剧了HTTP的性能问题
        - 因为当顺序发送的请求序列种的一个请求因为某种原因在服务端被阻塞时,在后面的排队的所有请求也一同被阻塞了,会导致客户端一直请求不到数据,这就是`队头阻塞`
- http1.1性能瓶颈
    - http头部巨大且重复
        - 含有很多固定的字段,比如Cookie,User Agent,Accept,这些字段加起来也可高达几百字节甚至上千字节,未经压缩就发送,所以有必要压缩
        - 大量的请求和响应的报文里有很多字段值都是重复的,这样会使大量带宽被这些冗余的数据占用了,所以有必须要避免重复性;
        - 由于HTTP协议是无状态的,每一个请求都得携带HTTP头部,特别是对有携带cookie的头部,而cookie的大小通常很大;
        - 字段是ASCll编码的,虽然易于认类的观察,但是效率低,所以有必要改成二进制编码
    - 并发连接有限
        - 谷歌浏览器最大并发连接数是6个,而且每一个都要经过TCP和TLS握手耗时,以及TCP慢启动过程给流量带来的影响
    - 队头阻塞问题
        - 同一连接只能完成一个HTTP事务(请求和响应)后,才能处理下一个事务
        - 服务器是按请求的顺序响应的,如果服务器响应慢,会招致客户端一致请求不到数据,也是队头阻塞
    - 没有请求优先级控制:
    - 不支持服务器推送消息
        - 请求只能从客户端开始,服务器只能被动响应
        - 因此当客户端需要获取通知时,只能同故宫定时器不断地拉取消息,浪费大量的带宽和服务器资源
- HTTP1.1如何优化    
    - 缓存技术尽量避免发送HTTP请求
      -缓存技术: 避免发送HTTP请求,客户端收到第一个响应收,可以将其缓存
    - 在需要发送HTTP请求时,考虑如何减少请求次数
    - 减少服务器的HTTP响应的数据大小
    - HTTP1.1一些关键地方无法优化
        - 这些手段都是对Http1.1协议的"外部"做优化,而一些关键的地方时没办法优化的,比如请求-响应模型,头部巨大且重复
            并发连接耗时,服务器不能主动推送等,要改变这些必须重新设计HTTP协议,于是HTTP/2就出来了
...
- 兼容HTTP1.1
- 概念: 
    - HTTP/2出来的目的是为了改善HTTP的性能
    - 协议升级有一个很重要的地方,就是要兼容老版本的协议,否则新协议推广起来就相当困难,所幸Http/2做到了兼容http/1.1
    
- 做法
    - HTTP/2 没有在URI里引入新的写一名,仍然哟个[http://]表示明文协议,用[https://]表示加密协议,用户意识不到协议的升级,实现了协议平滑升级
    - 只在应用层做了改变,还是基于TCP协议传输,应用层方面为了保持功能上的兼容,HTTP/2 把 http 分解成了 语义 和 语法 两个部分
        - 语义 层不做改动,与http1.1完全一致,比如请求方法,状态码,头字段等规则保留不变
        - http/2在语法层面做了很多改造,基本改变了HTTP报文的传输格式

- http2 性能上的该进
    - 头部压缩
        - http协议的报文由[header + body]构成
            - 多余body部分来说,http/1.1协议可以使用头字段[content-ecoding]指定body的压缩方式,比如用gzip压缩,这样可以几月带宽
        - 对于header部分,http/1.1是没有针对它的优化手段
    - http1.1报文header问题
        - 含有很多固定的字段,比如cookie,userAgent,Accept等,这些字段加起来也高达几百字节甚至上千字节,所以有必要压缩
        - 大量的请求和响应的报文里由很多字段值都是重复的,这样会使得大量带宽被这些冗余的数据占用了,所以必须要避免重复性
        - 字段是ASCll编码的,虽然易于人类观察,但效率地,所以有必要改成二进制编码
    - http/2 HPACK算法
        - 对于常见的http头部通过`静态表`和`huffman编码`的方式,将体积压缩了近一般,而且针对后续的请求头部,还可以`建立动态表`将体积压缩近90%,大大提高了编码效率,同时节约了带宽资源
        - http/2没有常见的gzip压缩方式来压缩头部,而是开发了hpack算法
            - HPACK算法
                - 概念: 在客户端和服务端同时维护一张头信息,所有字段都会存入这个表,生成一个索引表,以后就不发送同样字段了,只发送索引号,提高速度;  
                    客户端和服务器两端都会建立和维护[字段]用长度较小的素银好白哦是重复的字符串,在用Huffman编码压缩数据,可达到50%~ 90%的高压缩率
                - 组成  
                    - 静态字典
                        - http/2 为高频出现在头部的字符串和字段建立了一张静态表
                            它是写入到http/2狂啊及的不会变化的,静态表共有61组
                    - 动态字典 
                        - 静态表只包含了61中高频出现在头部的字符串,不再静态表范围的头部字符串就要自行构建动态表;它的index从`62`起步,会在编码节码的时候随时更新
                        - 第一次发送时头部中的[user-agent]字段数据上由上百个字节,通过Huffman编码发送后,客户端和服务器双方都会更新自己的动态表,添加一个新的index号62,那么在下一次发送的时候,就不用重复发送这个字段的数据了,只用发1字节的index号就好了,因为双方都可以根据自己的动态表获取到字段的数据
                        - 使得动态表生效有一个前提,必须同一个连接上,重复传输完全相同的http头部,如果消息字段在1个连接上只发送了1次,或者重复传输时,字段总是略有变化,动态表就无法被充分利用了
                        - 随着在同一http2连接上发送的报文越来越多,客户端和服务器双方的`字典`积累的越来越多,理论删最终每个头部字段都会变成1字节的index,这样便避免了大量的冗余数据的传输,大大节约了带宽
                    - 动态表问题
                        - 动态表并非无限增大,一位内动态表时会占用内存的,动态表越大,内存也越大,容易影响服务器总体的并发能力,因此服务器需要限制http/2连接时长或者请求次数,达到上限后,就会关闭http/2连接释放内存
                        - 动态表时具有时序性的,如果首次出现的请求发生了丢包,后续的收到请求,对方就无法编码出HPACK头部,因为对方还每建立号动态表,因此后续的请求编码会阻塞到首次请求中中丢失的数据包重传过来
                - Huffman编码(压缩算法)
                    - Huffman编码的原理时将高频出现的信息用[较短]的编码表示,从而能缩减字符串长度
                    - http/2根据出席那频率将ASCll码编码为了静态Huffman编码表
    - 二进制帧格式
      - 二进制
        - 将http1的文本格式改成二进制传输数据,极大提高了HTTP传输效率,而且二进制数据使用位运算能高效解析
        - 头信息和数据体都是二进制,并且统称位帧(frame):头部信息帧(Headers Frame)和数据帧(Data Frame)
        - 对然对人不友好,但是对计算机非常友好,因为计算机只懂二进制,那么收到报文后,无需再将明文的报文转换成二进制,而是直接解析二进制报文,这增加了数据传输的效率
      - 帧格式
        - 响应报文
            - http/2把响应报文划分成了两个帧,图中的HEADERS(首部,和DATA消息负载)时帧的类型,也就是说一条HTTP响应,划分了两个帧来传输,并且采用二进制来编码
        - Frame帧结构![img_24.png](img_24.png)
            - 帧头很小,只有9个字节
            - 帧数据: 它存放的时通过HPACK算法压缩过的HTTP头部和包体
    - 并发传输
        - http1.1队头阻塞问题
            - http1.1实现的时基于请求-响应模型的,同一连接中,HTTP完成了一个事务(请求与响应),才能处理下一个事务,也就是说再发出请求等待的过程中,是没办法做其他的事情的,
                如果响应迟迟不来,那么后续的请求是无法发送的,也就造成了队头阻塞的问题
            - HTTP/2引出了Steam
                - 概念: 
                    - http/2实现了Stream并发,当HTTP/2实现多个并发Stream时,只需要建立一次TCP连接, 而HTTP1.1需要建立多个TCP连接,每个TCP连接都要经过TCP握手,慢启动以及TLS握手过程
