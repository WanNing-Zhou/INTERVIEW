...
4. 判断返回值,返回对象就用该对象,没有的化就创建一个对象


### 83. Split() 和 join() 的区别

split()是把一串字符(根据某个分割符)分成若干个元素存放在一个数组里,即切割成数组的形式  
join()是把数组中的字符串连城一个长串,可以大体上认为Split()的逆操作

### 84. JavaScript如何对一个对象进行深度clone ?

将对象转换为JSON: `JSON.stringify(obj)`  
将JSON转换为对象: `JSON.parse(jsonObj)`

### 85. js数组去重方法

1. 使用ES6方法 [... new Set(arr)]
2. 利用新数组indexOf查找indexOf()方法可返回某个执行的元素在数组中首次出现的未知,如果没有就返回-1  
3. for 双重循环,通过判断第二层循环,去重的数组中是否含有该元素,如果有就退出第二层循环,如果没有 j == result.length 就相等然后把对应的元素添加到最后的数组里面  

### 86, javaScript 垃圾回收机制的理解  
...
### 88. eval()是做什么的
1. eval()的作用 
把字符串解析成JS代码并运行,并返回执行的结果

### 89 import 和 export 

ES6标准中,JavaScript原生支持模块(module);这种将js代码分割成不同功能的小块及逆行模块化,将不同功能的代码分别卸载不同文件中,各个模块只需导初公共接口部分,然后通过模块的导入方式可以在
其他地方使用

`export` 用于对外输出模块(一个文件可以理解为一个模块)变量的接口

`import` 用于在一个模块中加载另一个含有export接口的模块

import 和 export 命令只能在模块的顶部,不能再代码块中  


### 90. Promise对象

promise是异步编程的一种解决方案,将异步操作以同步操作的流程表达出来,为了避免层层嵌套的回调函数,要是为了解决异步处理回调地狱(也就是循环嵌套的问题)而产生的  

promise构造函数包含一个参数和一个带有resolve(解析)和reject(拒绝)两个参数的回调,再回调中执行一些操作(例如异步),如果一切都正常,则调用resolve,否则调用reject

对于已经实例化的promise对象可以调用Promise.then()方法,传递resolve否则调用reject方法作为回调,then()方法接收两个参数 onResolve 和 onReject,分别代表当前Promise对想再成功或失败时  


### 91. fetch 和 axios 

1. fetch没有办法原生检测请求的进度,而XHR可以
2. fetch不支持abort,不支持超时控制,使用setTimeout及Promse.reject的是西安的超时控制并不能阻止请求过程,继续再后台运行, 早成了资源德浪费
3. fetch默认不会带cookie,需要添加配置项,
4. fetch只对网络请求报错,对400,500都当作成功的请求,需要封装区处理摆脱了XHR,是ES规范里新的实现方式,更加底层;
提供的API更丰富,封号更方便的写法符合关注分离,没有输入,输出和用时间来跟踪的状态混杂再一个对象里
   

### 92. 禁止javaScirpt脚本读取cookie信息
...
http是一种无状态的协议,客户端与服务器建立连接并传输数据,数据传输完后,连接就会关闭;再次教书数据需要尽力新的连接,
因此,服务器无法从连接上跟踪会话,也无法知道用户上一次做了什么,这严重阻碍了基于web应用程序的交互,也影响用户的交互体验;

cookie是解决HTTP无状态性的有效手段,服务器可以设置或读取cookie中包含的信息,当用户登录后,服务器会发送包含登录凭据的Cookie到用户浏览器客户端;
而浏览器对该Cookie进行某种形式的存储,用户再次访问该网站时,浏览器会发送该cookie到服务器,服务器对该平居进行验证,合法时yoghurt不必输入用户名和密码就可以直接登录

cookie就是服务器记录用户状态的一种手段,cookie信息会保存用户状态;

### 94. 一个页面从输入URL到页面加载显示完成,这个过程中都发生了什么?  
...
请求原理  
HTTP 协议是应用层的一种协议，是一种 C/S 架构服务，基于 TCP/IP 协议来通信,监听再TCP的80端口上,HTTP协议实现的是用户端可以项服务端获取web资源


### 96 HTTPS 有几次握手和挥手？HTTPS 的原理什么是  
...
2. 当主线程中的执行栈为空时,检查时间队列是否为空,如果为空,则继续检查,如果不为空,则执行3  
3. 取出任务队列的首部,加入执行栈
4. 执行任务  
5. 检查执行栈,如果栈为空,则跳回第二部,如果不为空则继续检查



### 98 commonJS中的require/expots和ES6中 import/export 的区别时什么

**commonJs中的require/exports**  
commonJS模块的重要特性时加载时执行,及脚本代码在require的时候就会全部执行,一旦出现某个模块备"循环加载"就直输出
已经执行的部分,还没有执行的部分时不输出的  

**ES6中 import/export**  
ES6模块是动态引用,如果使用import从一个模块加载变量,那些变量不会缓存,而是成为一个执行备加载模块的引用, import/export最终都是编译为require/exports来执行的

### 99 浅谈promise与async/await的区别

**区别**  
1. 函数前面多了一个`async`关键字, await关键字只能用于async 定义的函数内,async会隐式地返回一个`promise`该promise的resolve值就是return值,
...
2. `await`的使用是由限制的,await关键字只能用于async定义的函数内,如果未使用async而直接使用await就会抛SynctaxError


**异常处理**  
Promise对象提供catch方法对异常处理,async在异常处理中通过`try{}catch(){}`处理

**代码结构上**  
`async/await`更接近于同步的写法;promise中引入then,catch的方法,代码更显冗余  

`async/await`结合中大量的await的promises相继发生会使程序变慢


async和promise都是异步方法,区别使async生成的结果是promise对象,async是promise的终结半,await只能在async中使用,await是阻塞的意思,
就是暂停,

使用async/await并不需要回调嵌套,也不需要写多个then方法,在使用上甚至类似一个同步操作


> promise的出现解决了传统callback函数导致的回调地狱问题,但是它的语法导致它纵向发展形成了一个回调链,遇到了复杂的业务场景线然是不美观的  
> 
...
4. 开源数据库的的分额在不断增加,mysql的份额也不断持续增加
5. 缺点就是海量数据处理的时候效率显著变慢

**MongoDB**  
非关系型数据库(Nosql),属于文档型数据库(文档型数据库:即可存放xml,json,类型的数据,这些数据具备自述3型,册灰姑娘先分层的树状数据结构,数据结构由键值对组成)  
...
**数据处理**: 数据是存储在硬盘上的,只不过需要经常读取数据才会被加载到内存中,将数据存储在物理内存中,从而达到高速读写  
...
1. 不支持事务操作  
2. 占用空间过大  
3. mongoDB没有用 MySQL 那样成熟的维护工具  
4. 无法进行关联表查询,不适用关系多的数据
5. 复杂聚合操作总通过 mapreduce创建,速度慢
